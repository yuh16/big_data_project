...
#            self.parameters += [fc3w, fc3b]
        ...
        # fc3
        with tf.name_scope('fc3') as scope:
            fc3w = tf.Variable(tf.truncated_normal([4096, 21],
                                                         dtype=tf.float32,
                                                         stddev=1e-1), name='weights')
            fc3b = tf.Variable(tf.constant(1.0, shape=[21], dtype=tf.float32),
                                 trainable=True, name='biases')
            fc3l = tf.nn.bias_add(tf.matmul(self.fc2, fc3w), fc3b)
...
if __name__ == '__main__':
    ...
    labels = tf.placeholder(tf.float32, shape=(1, 21))
    prob1 = tf.placeholder(tf.float32, shape=(1, 21))
    keep_var = tf.placeholder(tf.float32)
    vgg = vgg16(imgs, 'vgg16_weights.npz', sess)
    global_step = 0.01
    i=1
    class_names2 = class_names[1:-1]
    pred = vgg.probs
    ...
    for dirpath, dirnames, filenames in os.walk("./new_data"):
        for filename in filenames:
            if os.path.splitext(filename)[1] == '.jpg':
                ...
                labels1[0,class_names2.index(labelstring)] = 1
                sess.run(optimizer,feed_dict={vgg.imgs: img1, labels: labels1})
                acc = sess.run(accuracy, feed_dict={vgg.imgs: img1, labels: labels1})
                print "Fine-tuning picture %d" %i
                ...
                p = preds[0]
                #for p in preds:
                print class_names2[p], prob[0], labelstring
